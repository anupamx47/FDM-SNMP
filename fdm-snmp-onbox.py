	  	...
	  	for response in responses:
		  	for interface in response.json()['items']:
		  		if interface['name'] is not None and interface['name'] !='':
		  			...
		  			interface_counter+=1
	  	interface_selection=int( raw_input("Select the Phyinterface (Integer value only) : "))
...
#device =  raw_input("Enter the device IP address: ")
device="127.0.0.1"
username =  raw_input("Enter the username of the FTD: ")
password = getpass.getpass("Enter the password of the FTD: ")


token=auth()

ver_flag=int( raw_input('Would you like to configure (1) SNMPv2 or  (2) SNMPv3 : ' ))

name= raw_input("Enter the SNMP Server object name : ")
ip=  raw_input("Enter the SNMP Server object IP : ")
...
if ver_flag==1:
	...
	community_str= raw_input('Enter SNMPv2 community string : ')
	sec_Configuration= {
	...
	snmpv3_payload['name'] =  raw_input('Enter SNMPv3 username : ')
	snmpv3_payload['securityLevel'] =  raw_input("Enter Security Level => Options ['AUTH', 'NOAUTH', 'PRIV'] :  ")

	if snmpv3_payload['securityLevel'] in ['AUTH','PRIV']:
		snmpv3_payload['authenticationAlgorithm'] =  raw_input("Enter authentication Algorithm => Options ['SHA', 'SHA256'] : ")
		snmpv3_payload['authenticationPassword']= raw_input("Enter authentication password : ")

	if snmpv3_payload['securityLevel'] == "PRIV":
		snmpv3_payload['encryptionAlgorithm']=  raw_input("Enter encryption Algorithm => Options ['AES128', 'AES192', 'AES256', '3DES'] : ")
		snmpv3_payload['encryptionPassword']= raw_input("Enter encryption password : ")
...
snmp_hostname= raw_input('Enter SNMP host object name : ')
